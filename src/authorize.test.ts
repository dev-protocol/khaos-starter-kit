...
test('Returns false if either the passed message or secret is not string type', async (t) => {
	const [res1, res2, res3] = await Promise.all([
		authorize({ message: 1 as any, secret: 'test', req: {} as any }),
		authorize({ message: 'test', secret: 1 as any, req: {} as any }),
		authorize({ message: 1 as any, secret: 1 as any, req: {} as any }),
	])
	t.false(res1)
	t.false(res2)
	t.false(res3)
